...
namespace Hera.Helpers
{
    public static class Utils
    {
        ...
        public static int CountOfAddsInRange(double distance, WoWPoint location)
        {
            ...
            {
                if (location.Distance(u.Location) <= distance) nearLocation++;
            }
        ...
        public static bool CanAoEInstance
        {
            get
            {
                ...
                foreach (WoWUnit u in hlist)
                {
                    if (u.HealthPercent < 25) continue;                                         // If the mobs health is below 25% don't include it
                    if (RaFHelper.Leader.Location.Distance(u.Location) < 15) countNearTank++; 
                }
        ...
        public static bool IsInLineOfSight()
        {
            if (Me.IsInParty)
            {
                if (RaFHelper.Leader != null && !Utils.IsInLineOfSight(RaFHelper.Leader.Location)) return false;
            }
            else
            {
                if (Me.GotTarget && !Utils.IsInLineOfSight(CT.Location)) return false;
            }
