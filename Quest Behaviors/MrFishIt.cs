...
namespace Honorbuddy.Quest_Behaviors.MrFishIt
{
    ...
    internal class MrFishIt : QuestBehaviorBase
    {
        ...
        private readonly WaitTimer _baitTimer = WaitTimer.FiveSeconds;
        private readonly WaitTimer _buffTimer = WaitTimer.FiveSeconds;
        ...
        private static readonly List<Lure> s_lures = new List<Lure>
                                                    {
        ...
        private static readonly Dictionary<uint, string> s_baits = new Dictionary<uint, string>
                                                                   {
        ...
        private static readonly Dictionary<uint, string> s_buffItems = new Dictionary<uint, string>
                                                                   {
        ...
        private async Task<bool> ApplyBait()
        {
            ...
            var bait = StyxWoW.Me.BagItems.FirstOrDefault(
                    i => HasRequiredSkillLevel(i.ItemInfo) && (BaitId.HasValue ? BaitId.Value == i.Entry : s_baits.ContainsKey(i.Entry)));
        ...
        private async Task<bool> ApplyItemBuff()
        {
            ...
            var buffItem = Me.BagItems.FirstOrDefault(i =>
            {
                ...
                if (!s_buffItems.TryGetValue(i.Entry, out buffName))
                    return false;
        ...
        private static WoWItem GetBestLure()
        {
            return (from item in StyxWoW.Me.BagItems
                    ...
                    let lure = s_lures.FirstOrDefault(l => l.ItemId == item.Entry)
                    where lure != null
        ...
        private static int GetBonusFishingSkillOnEquip(WoWItem item)
        {
            ...
            {
                var bestLure = GetBestLure();
                var bestLureBonus = bestLure != null ? ((int?)s_lures.First(l => l.ItemId == bestLure.Entry).BonusSkill) : null;
        ...
        public static Composite CreateMoveToPoolBehavior()
        {
            return new Decorator(ret => Pool != null && !Blacklist.Contains(Pool, BlacklistFlags.All),
                new Sequence(
                    ...
                    new Action(ret => s_movetopoolTimer.Start()),
