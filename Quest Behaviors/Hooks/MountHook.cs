...
namespace Honorbuddy.Quest_Behaviors.Hooks
{
    ...
    public class MountHook : CustomForcedBehavior
    {
        ...
        //Used to work without this, then something happenddddddddd
        //dirty imo too
        private static Composite IwRunner
        {
            get
            {
                return new PrioritySelector(new HookExecutor("Loot_Main"), new HookExecutor("Combat_Main"), new HookExecutor("Combat_Only"), new HookExecutor("Death_Main"), new HookExecutor("Questbot_Main"));
            }
        }
        private static Composite runQuestOrder
        {
            get
            {
                //
                return new Decorator(r => customQuestOrder != null, new PrioritySelector(new Decorator(r => customQuestOrder.CurrentNode == null, new Action(r => CleanupQuestOrder())), new HookExecutor("GoobyHook2"), IwRunner, new ActionAlwaysSucceed()));
            }
        }
        ...
        private static void SetupQuestLowbieOrder()
        {
            ...
            customQuestOrder = new QuestOrder();
            ...
            customQuestOrder.Nodes = new OrderNodeCollection(Profile.QuestOrder.Count);
            customQuestOrder.Nodes.AddRange(Profile.QuestOrder);
            customQuestOrder.Nodes.IgnoreCheckpoints = Profile.QuestOrder.IgnoreCheckpoints;
            customQuestOrder.UpdateNodes();
            //customQuestOrder.CurrentBehavior = null;
            QOrunner = new ForcedBehaviorExecutor(customQuestOrder);
            TreeHooks.Instance.AddHook("GoobyHook2", QOrunner);
            setupQO = false;
        }
        ...
        private static void SetupQuestFlyingOrder()
        {
            ...
            customQuestOrder = new QuestOrder();
            //Replace with memorystream once profile is finalized    
            //var reader = new StreamReader(File.OpenRead(@"C:\Users\Dennis\Desktop\New folder\hbx\FlyingTraining.XML"));
        ...
        private static Composite PurchaseMount
        {
            ...
            get
            {
                return new PrioritySelector(
                    new Decorator(r => FlightLevel == 1 && !Mount.GroundMounts.Any(), new Action(r => SetupQuestLowbieOrder())),
                    new Decorator(r => FlightLevel == 2 && !Mount.FlyingMounts.Any(), new Action(r => SetupQuestFlyingOrder()))
        ...
        public static Composite myHook
        {
            get
            {
                if (_myHook == null)
                {
                    _myHook = new Decorator(r => !Me.Combat, new PrioritySelector(new Decorator(r => setupQO, new Action(r => SetupQuestFlyingOrder())), runQuestOrder, runOtherComposite, PurchaseMount, OldWordComposite, HellfireComposite));
                    return _myHook;
                }
                return _myHook;
            }
