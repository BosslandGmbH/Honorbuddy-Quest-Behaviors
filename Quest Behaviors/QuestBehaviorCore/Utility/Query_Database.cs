...
namespace Honorbuddy.QuestBehaviorCore
{
    ...
    public class Query_Database
    {
        ...
        private static readonly HashSet<int> s_vendorBlacklist = new HashSet<int>();

        private static readonly SQLiteCommand s_sqlCmd_NearestVendor =
            BuildSqLiteCommand("SELECT * FROM npcs"
                                + "WHERE MapId = @MAP_ID AND flag & @FLAG"
                                + " ORDER BY VECTORDISTANCE(x,y,z,@X,@Y,@Z) ASC");

        private static readonly SQLiteCommand s_sqlCmd_NearestMailbox =
