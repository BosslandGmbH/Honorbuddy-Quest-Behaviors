...
namespace Honorbuddy.QuestBehaviorCore
{
    ...
    internal class SpellCastMonitor : IDisposable
    {
        ...
        private static readonly Dictionary<string, string> s_localizedSymbols = new Dictionary<string, string>();
        ...
        {
            string ret;
            if (s_localizedSymbols.TryGetValue(symbol, out ret))
                return ret;

            ret = Lua.State.Globals.GetField(symbol).Value.String.Value;
            s_localizedSymbols.Add(symbol, ret);
            return ret;
        }
