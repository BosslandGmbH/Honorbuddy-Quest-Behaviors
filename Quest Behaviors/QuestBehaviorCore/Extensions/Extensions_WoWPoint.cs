...
namespace Honorbuddy.QuestBehaviorCore
{
    public static class Extensions_WoWPoint
    {
        ...
        {
            private static readonly TimeSpan s_maxCacheTimeSpan = TimeSpan.FromMilliseconds(2500);

            private static DateTime s_lastCleanupTime = DateTime.MinValue;

            // a list is probably the best collection to use if avg collection size is small (less than 20 to 30 items)
            private static readonly List<SurfacePathDistanceCache> s_pathDistanceCache = new List<SurfacePathDistanceCache>();
            ...
            {
                s_pathDistanceCache.Add(new SurfacePathDistanceCache(start, destination, distance));
            }
        }
