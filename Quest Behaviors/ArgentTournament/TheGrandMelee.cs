...
namespace Styx.Bot.Quest_Behaviors
{
    ...
    public class TheGrandMelee : CustomForcedBehavior
    {
        ...
        private HashSet<uint> _mobIds_AllianceMounts = new HashSet<uint> {33790, // Stabled Exodar Elekk
        ...
        private HashSet<uint> _mobIds_NeutralMounts = new HashSet<uint> {33842, // Stabled Sunreaver Hawkstrider
																		33843, // Stabled Quel'dorei Steed
																		};


        private uint[] _enemy;// = new uint[] { 33384, 33306,33285,33382,33383};
        private uint[] _enemyDebuff;// = new uint[] { 64816, 64811, 64812, 64813, 64815 };
        ...
        {
            var isMount = (Me.IsHorde ? _mobIds_HordeMounts.Contains(unit.Entry) : _mobIds_AllianceMounts.Contains(unit.Entry))
                          || _mobIds_NeutralMounts.Contains(unit.Entry);
        ...
        private WoWUnit WhichNPC
        {
            get
            {
                return ObjectManager.GetObjectsOfType<WoWUnit>()
                    .Where(x => x.IsAlive && _debuffs.ContainsKey(x.Entry) && !Me.HasAura((int)_debuffs[x.Entry]))
        ...
        public override void OnStart()
        {
            ...
            if (!IsDone)
            {
                ...
                {
                    _debuffs.Add(_enemy[i], _enemyDebuff[i]);
                }
